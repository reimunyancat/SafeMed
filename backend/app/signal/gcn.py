"""Co-medication graph metrics with optional trained-GCN embedding boost.

Two modes:

1. **Heuristic mode (default, no extra deps)** — The user's regimen is treated
   as a clique; degree centrality + a saturating size term yield a per-drug
   risk amplifier in [0, 1]. This is what runs on Railway with no ML extras.
2. **Trained-embedding mode** — If a ``gcn_embeddings.pt`` file produced by
   ``app.ml.train`` is available (``GCN_EMBEDDING_PATH`` env var or the default
   ``data/processed/gcn_embeddings.pt``), we additionally boost amplifiers for
   ingredients that sit close to each other in the learnt embedding space
   (cosine similarity), which captures pharmacologically similar ingredients
   that don't appear in the literal MFDS contraindication CSV.

The heuristic alone is what the competition demo runs on; the trained boost
is available for the Phase B writeup and live tuning.
"""
...
def _load_embeddings() -> tuple[dict[str, int], object] | None:
    ...
    The function is cached so repeated /analyze calls don't re-read the .pt
    file. We import torch lazily so the runtime image without ML extras works.
    """
...
def _embedding_boost(drugs: list[str]) -> dict[str, float]:
    ...
    Returns zeros if no trained embeddings are loaded or none of the drugs are
    in the vocab.
    """
    ...
    for i, (drug_a, idx_a) in enumerate(items):
        for drug_b, idx_b in items[i + 1 :]:
            ...
            # Map cosine [-1, 1] -> boost [0, 1], emphasising strong similarity.
...
def score_drugs(
    ...
    n = len(drugs)
    amp_base = min(1.0, (n - 1) / 6.0)  # saturates at 7+ meds
    boosts = _embedding_boost(drugs)
    out: list[GCNScore] = []
    for d in drugs:
        base = float(amp_base * centrality[d])
        boost = float(boosts.get(d, 0.0))
        # Combine multiplicatively but cap at 1.0; embedding boost can lift
        # the amplifier by up to 50%.
    ...
    # silence the unused-import warning when math is only used in some paths
    _ = math
    return out
